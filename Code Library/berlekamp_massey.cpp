...
LL RDB_generator(int level) {
    ...
    for (int l = 1; l < level; l++) {
        for (auto x: nodes) {
            ...
            else if ((int)graph[x].size() == 1) {
                graph[x].push_back(++id); nodes.push_back(id);
                graph[x].push_back(++id); nodes.push_back(id);
            }
        }
    }
    ...
    for (int i = (int)nodes.size() - 1; i >= 0; i--) {
        if (done[nodes[i]]) continue;
        if ((int)graph[nodes[i]].size() != 3) continue;
        bool f = true;
        for (auto x: graph[nodes[i]]) if (done[x]) f = false;
        if (f) {
            done[nodes[i]] = true;
            claw += 4;
            for (auto x: graph[nodes[i]]) done[x] = true;
        }
    }
 
    //cout << "claws for level " << level << " " << claw << endl;
    return claw;
}
 
vector <LL> v = {0, 0, 4, 4, 12, 24, 48, 96, 196, 388};
